...
def update_stat(chat_id, status_type, delta=1):
    """Cập nhật thống kê an toàn, chống lỗi xung đột cấu trúc cột giữa các phiên bản SQLite"""
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    try:
        # Kiểm tra xem chat_id này đã có trong bảng statistics chưa
        cur.execute("SELECT chat_id FROM statistics WHERE chat_id = ?", (chat_id,))
        row = cur.fetchone()
        
        if not row:
            # Nếu chưa có thì chèn mới với giá trị ban đầu là 0
            cur.execute(
                "INSERT INTO statistics (chat_id, delivered_count, delivering_count, cancelled_count) VALUES (?, 0, 0, 0)", 
                (chat_id,)
            )
        
        # Tiến hành cộng dồn số lượng theo loại trạng thái tương ứng
        if status_type == "delivered":
            cur.execute("UPDATE statistics SET delivered_count = MAX(0, delivered_count + ?) WHERE chat_id = ?", (delta, chat_id))
        elif status_type == "delivering":
            cur.execute("UPDATE statistics SET delivering_count = MAX(0, delivering_count + ?) WHERE chat_id = ?", (delta, chat_id))
        elif status_type == "cancelled":
            cur.execute("UPDATE statistics SET cancelled_count = MAX(0, cancelled_count + ?) WHERE chat_id = ?", (delta, chat_id))
            
        conn.commit()
    except Exception as e:
        logging.error(f"Lỗi khi cập nhật bảng thống kê cho {chat_id}: {e}")
    finally:
        conn.close()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "waiting_name":
        ...
        # Cập nhật số lượng vào bảng thống kê một cách an toàn
    ...
    for chat_id, tracking, json_str in rows:
        ...
        if new_status and new_status != old_status:
            ...
            if old_type != new_type:
                update_stat(chat_id, old_type, delta=-1)
                update_stat(chat_id, new_type, delta=1)
...
def main():
    ...
    print("🚀 [SUCCESS] SPX Telegram Bot v3 ổn định đã hoạt động...")
    app.run_polling()
