...
ADMIN_ID = 5475751501  # <--- SỬA LẠI: Thay số này bằng ID Telegram thật của bạn
...
def check_spx_status(tracking):
    ...
    try:
        ...
        # ✨ FIX LỖI SAI TIME: Ép timestamp sang đúng múi giờ Asia/Ho_Chi_Minh của Việt Nam
        actual_time = latest.get("actual_time")
        if actual_time:
            dt_utc = datetime.fromtimestamp(int(actual_time), tz=pytz.utc) # Chuyển timestamp về giờ chuẩn quốc tế
            dt_vn = dt_utc.astimezone(VN_TZ) # Ép ngược sang múi giờ Việt Nam (+7)
...
# ================= ⏳ AUTOMATIC STATUS CHECK JOB (GIAO DIỆN VIP MỚI) =================
...
async def auto_check_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id, tracking, json_str in rows:
        ...
        if new_status and new_status != old_status:
            ...
            # ✨ THAY ĐỔI GIAO DIỆN THEO YÊU CẦU: Tên đơn viết hoa làm TIÊU ĐỀ + Thêm Icon
            try:
